...
func OnGetBattlePayConfig(s *Session, body []byte) ([]byte, error) {
	...
	goldCostBoosters := []*hsproto.PegasusUtil_GoldCostBooster{}
	cost := []ProductGoldCost{}
	db.Where("pack_type != ?", 0).Find(&cost)
	for _, costs := range cost {
		goldCostBoosters = append(goldCostBoosters, &hsproto.PegasusUtil_GoldCostBooster{
	...
	for _, bundle := range bundles {
		bundleItems := []*hsproto.PegasusUtil_BundleItem{}
		products := []Product{}
...
func OnPurchaseWithGold(s *Session, body []byte) ([]byte, error) {
	req := hsproto.PegasusUtil_PurchaseWithGold{}
	...
	res := hsproto.PegasusUtil_PurchaseWithGoldResponse{}
	// TODO: Query the DB to ensure we have enough gold
	result := hsproto.PegasusUtil_PurchaseWithGoldResponse_PR_SUCCESS
	res.Result = &result
